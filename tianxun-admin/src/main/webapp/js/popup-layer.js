...
var PopupLayer = new Class({
	...
	_init:function(options){
		...
		//var __width = this.trigger.width();alert(__width);
		this.B
	...
	_construct:function(){                  //构造弹出层
		this.popupBlk.show();
		this.popupLayer.append(this.popupBlk.css({opacity:1})).appendTo($(document.body)).css({position:"absolute",'z-index':2,width:this.trigger.parent('div').width() + 5.5,height:this.popupBlk.get(0).offsetHeight});//width:this.popupBlk.get(0).offsetWidth
	...
	recalculatePopupIframe:function(){     //重绘popupIframe,这个不知怎么改进，只好先用这个笨办法。
		this.popupIframe.css({position:"absolute",'z-index':-1,left:0,top:0,opacity:0,width:this.trigger.parent('div').width() + 5.5,height:this.popupBlk.get(0).offsetHeight});
	},
	close:function(){                      //关闭方法
	...
	init:function(pop){
		var on = false;
    	$(this.options.trigger).blur(function(){
    		if (!on) {
    			pop.close();
    		}
    	});
    	$('div.popupLayer').mouseleave(function(){
    		pop.close();
    		on = false;
    	});
    	$('div.popupLayer').mouseover(function(){
    		on = true;
    	});
	}
});